...
export type CopyTask = { id: string; name: string; repositoryId: string; destinationRoot?: string; sourceUuid: string; status: 'queued' | 'running' | 'verifying' | 'paused' | 'completed' | 'failed'; totalBytes: number; copiedBytes: number; speed: number; eta: number | null; history: number[]; verifyStatus: string; verifiedBytes: number; verifySpeed: number; verifyEta: number | null; verifyHistory: number[]; verifyError: string; files: CopyFileState[]; error?: string; createdAt: string; updatedAt: string };
export type BackgroundTask = { id: string; kind: 'scan' | 'index' | string; title: string; detail: string; status: 'running' | 'paused' | 'completed' | 'failed' | 'cancelled'; current: number; total: number | null; error: string; startedAt: string; updatedAt: string };
export type Settings = { foregroundScanMs: number; backgroundScanMs: number; askBeforeScan: boolean; notifications: boolean; keepRunning: boolean; ffmpegPath?: string; preventSleepCopy: boolean; preventSleepScan: boolean; preventSleepMapping: boolean; preventSleepApp: boolean };
...
export type CopyRequest = { name: string; sourceUuid: string; repositoryId: string; destinationRoot?: string; selection: { fileIds?: string[]; extensions?: string[]; startDate?: string; endDate?: string }; pathTemplate: string; note: string; mode: 'flat' | 'original' };
...
declare global {
  interface Window {
    materialGater: {
      ...
      confirmAction(options: { title: string; message: string; okLabel?: string; kind?: 'info' | 'warning' | 'error' }): Promise<boolean>;
      ...
      pauseCopyTask(id: string): Promise<AppState>; resumeCopyTask(id: string): Promise<AppState>; clearCompletedCopyTasks(): Promise<AppState>; clearFinishedCopyTasks(): Promise<AppState>; dismissCopyTask(id: string): Promise<AppState>;
      pauseBackgroundTask(id: string): Promise<BackgroundTask[]>; resumeBackgroundTask(id: string): Promise<BackgroundTask[]>; clearCompletedBackgroundTasks(): Promise<BackgroundTask[]>; clearFinishedBackgroundTasks(): Promise<BackgroundTask[]>; dismissBackgroundTask(id: string): Promise<BackgroundTask[]>;
